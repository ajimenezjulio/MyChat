//
//  FUser.swift
//  - Class for saving and loading Firebase Users
//  myChat
//
...
class FUser {
    ...
    init(_dictionary: NSDictionary) {
        ...
        if let created = _dictionary[kCREATEDAT] {
            // If format of date less than 14 characters, something's wrong
    ...
    class func loginUserWith(email: String, password: String, completion: @escaping (_ error: Error?) -> Void) {
        ...
        // Authentification in firebase, we use two variables one for the retrieved user
...
//    }
    ...
    class func logOutCurrentUser(completion: @escaping (_ success: Bool) -> Void) {
        ...
        do {
            ...
            // If everything ok (on completion) pass back a true (to the success variable)
            completion(true)
            
        } catch let error as NSError {
            // If error pass back the error and print it
            completion(false)
...
func fetchCurrentUserFromFirestore(userId: String) {
    ...
    reference(.User).document(userId).getDocument { (snapshot, error) in
        ...
        if snapshot.exists {
            ...
            // If everything ok, we save the user locally
...
// Update user values in firestore
func updateCurrentUserInFirestore(withValues : [String : Any], completion: @escaping (_ error: Error?) -> Void) {
    ...
    if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
        ...
        // Make mutable our actual dictionary stored in memory so it can update values
        let userObject = (dictionary as! NSDictionary).mutableCopy() as! NSMutableDictionary
        // Update values withe the new ones
        userObject.setValuesForKeys(tempWithValues)
        
        // Update user in firebase based on userId
        reference(.User).document(currentUserId).updateData(withValues) { (error) in
            
            if error != nil {
                // If error in firebase update, pass it back
                completion(error)
                return
            }
            ...
            // If error in some step, pass it back
            completion(error)
        }
